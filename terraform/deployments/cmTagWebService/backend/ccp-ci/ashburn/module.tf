terraform {
  backend "s3" {
    ...
    key    = "coast/ccp-ci/dataService/tagui/ccp-as-az1/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "ccp-ci-as-tagui" {
      ...
      runcmd:
          - |
            ...
            if [ -f TRIGGER_FILE ]; then
                ...
                cd "$(\ls -1dt ./*/ | head -n 1)"
                ansible-playbook -vvvv --connection=local --inventory "127.0.0.1," cmTagWebService.yml --extra-vars "env_type=ci datacenter=ta run_number=b role_id=${var.role_id} secret_id=${var.secret_id}" > local_2nd_ansible_run.txt
