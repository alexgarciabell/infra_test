terraform {
  backend "s3" {
    ...
    key    = "coast/ci/nlb/cmTagWebService/us-west-2/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "ci-west-cmtagws-nlb" {
  source                           = "../../../../../modules/nlb"
  ...
  description                      = "NLB For cmTagWebService - CI"
...
#Workspace naming format is tagportal_nlb_dw_(base date)_(iteration number)