terraform {
  backend "s3" {
    ...
    key    = "coast/staging/hatsmaker/cmTagWebService/us-east-1/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "ci-east-cmtagws" {
  ...
  security_groups              = ["sg-74b49806", "sg-01ce77aea6663f4b9", "sg-13387267"]
...
#Workspace naming format is tagportal_lb_de_(base date)_(iteration number)