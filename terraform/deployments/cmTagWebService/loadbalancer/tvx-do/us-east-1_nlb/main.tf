terraform {
  backend "s3" {
    ...
    key    = "coast/ci/nlb/cmTagWebService/us-east-1/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "ci-east-cmtagws-nlb" {
  source                           = "../../../../../modules/nlb"
...
#Workspace naming format is tagportal_nlb_de_(base date)_(iteration number)