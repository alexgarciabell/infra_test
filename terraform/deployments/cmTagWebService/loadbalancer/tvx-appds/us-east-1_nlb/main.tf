terraform {
  backend "s3" {
    ...
    key    = "coast/prod/nlb/cmTagWebService/us-east-1/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "prod-east-cmtagws-nlb" {
  source                           = "../../../../../modules/nlb"
...
#Workspace naming format is tagportal_nlb_ae_(base date)_(iteration number)