terraform {
  backend "s3" {
    ...
    key    = "coast/tvx-do/dataService/epsadmin/us-west-2-asg/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "ci-autoscaling-west-epsadmin" {
  source                      = "../../../../../modules/autoscaling/with_no_nlb"
  aws_instance_type           = "c5.xlarge"
  aws_security_groups         = ["sg-28e5374f", "sg-6776cd02", "sg-f57d2d91", "sg-b0000dcc", "sg-d8da89a5"]
...
#Workspace naming format is epsadmin_(version)_dw_asg_(iteration number)