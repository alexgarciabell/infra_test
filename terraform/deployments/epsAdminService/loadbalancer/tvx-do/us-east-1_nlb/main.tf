terraform {
  backend "s3" {
    ...
    key    = "coast/ci/nlb/epsadmin/us-east-1/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "ci-east-epsadmin-nlb" {
  source                           = "../../../../../modules/nlb"
...
#Workspace naming format is epsadmin_nlb_de_(base date)_(iteration number)