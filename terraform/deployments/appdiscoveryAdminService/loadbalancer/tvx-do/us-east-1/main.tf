terraform {
  backend "s3" {
    ...
    key    = "coast/staging/hatsmaker/adsadmin/us-east-1/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}
...
#Workspace naming format is adsadmin_lb_de_(base date)_(iteration number)