terraform {
  backend "s3" {
    ...
    key    = "coast/ccp-prod/dataService/adsadmin/ccp-as-az1/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "ccp-prod-as-adsadmin" {
      ...
      runcmd:
          - |
            ...
            if [ -f TRIGGER_FILE ]; then
                ...
                cd "$(\ls -1dt ./*/ | head -n 1)"
                ansible-playbook -vvvv --connection=local --inventory "127.0.0.1," appdiscoveryAdminService.yml --extra-vars "env_type=prod datacenter=pa service_listen_port_ssl=443 java_package=java-1.8.0-openjdk.x86_64 run_number=b" > local_2nd_ansible_run.txt
