terraform {
  backend "s3" {
    ...
    key    = "coast/staging/hatsmaker/configds/us-east-1-ingest/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}
...
module "ci-east-configds" {
  ...
  hats_id                      = "APPDS_DO_CONFIGSERVICE_INGEST_CI"
...
#Workspace naming format is xreconfigds_ingest_lb_de_(base date)_(iteration number)
