terraform {
  backend "s3" {
    ...
    key    = "coast/staging/hatsmaker/configds/us-west-2/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}
...
module "ci-west-configds" {
  ...
  security_groups              = ["sg-28e5374f", "sg-6776cd02", "sg-b0000dcc", "sg-f57d2d91"]
  ...
  hats_id                      = "APPDS_DW_CONFIGSERVICE_CI"
...
#Workspace naming format is xreconfigds_lb_dw_(base date)_(iteration number)