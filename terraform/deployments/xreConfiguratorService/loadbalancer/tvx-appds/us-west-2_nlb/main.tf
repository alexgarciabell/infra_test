terraform {
  backend "s3" {
    ...
    key    = "coast/prod/nlb/configuratorService/us-west-2/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "prod-west-configds-nlb" {
  source                           = "../../../../../modules/nlb"
  ...
  description                      = "NLB For configuratorService DS - PROD"
...
#Workspace naming format is xreconfigds_nlb_aw_(base date)_(iteration number)