terraform {
  backend "s3" {
    ...
    key    = "coast/prod/hatsmaker/configuratorService/us-west-2/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "prod-west-configds" {
  ...
  security_groups              = ["sg-8c46e3f0", "sg-e5cbb89f", "sg-affd71d5", "sg-06e3db685a3dd2d86"]
  ...
  hats_id                      = "APPDS_AW_CONFIGSERVICE_PROD"
...
#Workspace naming format is xreconfigds_lb_aw_(base date)_(iteration number)