terraform {
  backend "s3" {
    ...
    key    = "coast/prod/hatsmaker/customSplitEvaluator/us-west-2/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}
...
module "prod-west-cseval" {
  ...
  security_groups              = ["sg-0c682788507a751ac", "sg-e5cbb89f", "sg-affd71d5"]
...
#Workspace naming format is cse_lb_aw_(base date)_(iteration number)