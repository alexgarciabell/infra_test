terraform {
  backend "s3" {
    ...
    key    = "coast/prod/nlb/customSplitEvaluator/us-east-1/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "prod-east-customSplitEvaluator-nlb" {
  source                           = "../../../../../modules/nlb"
...
#Workspace naming format is cse_nlb_ae_(base date)_(iteration number)