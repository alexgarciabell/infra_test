terraform {
  backend "s3" {
    ...
    key    = "coast/prod/hatsmaker/customSplitEvaluator/us-east-2/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}
...
module "prod-ohio-cseval" {
  ...
  security_groups              = ["sg-0c44547d8d446517a", "sg-6ed1db05", "sg-d6a544bc"]
...
#Workspace naming format is cse_lb_ao_(base date)_(iteration number)