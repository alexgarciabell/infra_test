terraform {
  backend "s3" {
    ...
    key    = "coast/tvx-ingrid-adproxy/dataService/blueocean/prod-tsf-us-east-1-asg/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "prod-tsf-autoscaling-east-blueocean" {
  source                      = "../../../../../modules/autoscaling/with_no_nlb_profile"
  aws_instance_type           = "c5.xlarge"
  aws_security_groups         = ["sg-04285519007852828"]
