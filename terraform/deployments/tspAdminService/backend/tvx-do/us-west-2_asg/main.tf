terraform {
  backend "s3" {
    ...
    key    = "coast/tvx-do/dataService/tspadmin/us-west-2-asg/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "ci-autoscaling-west-tspadmin" {
...
#Workspace naming format is tspadmin_(version)_dw_asg_(iteration number)