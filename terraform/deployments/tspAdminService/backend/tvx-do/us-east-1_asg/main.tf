terraform {
  backend "s3" {
    ...
    key    = "coast/tvx-do/dataService/tspadmin/us-east-1-asg/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "ci-autoscaling-east-tspadmin" {
...
#Workspace naming format is tspadmin_(version)_de_asg_(iteration number)
