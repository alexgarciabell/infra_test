terraform {
  backend "s3" {
    ...
    key    = "coast/tvx-appds/dataService/tspadmin/us-west-2-asg/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "prod-autoscaling-west-tspadmin" {
...
#Workspace naming format is tspadmin_(version)_aw_asg_(iteration number)