terraform {
  backend "s3" {
    ...
    key    = "coast/tvx-appds/dataService/tspadmin/us-east-1-asg/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "prod-autoscaling-east-tspadmin" {
...
#Workspace naming format is tspadmin_(version)_ae_asg_(iteration number)