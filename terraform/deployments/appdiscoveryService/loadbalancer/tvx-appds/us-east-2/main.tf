terraform {
  backend "s3" {
    ...
    key    = "coast/prod/hatsmaker/appdiscoveryService/us-east-2/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}
...
module "prod-ohio-adsds" {
  ...
  security_groups              = ["sg-03c42d3d882ef759f", "sg-6ed1db05", "sg-d6a544bc"]
...
#Workspace naming format is adsds_lb_ao_(base date)_(iteration number)