terraform {
  backend "s3" {
    ...
    key    = "coast/prod/nlb/appdiscoveryService/us-east-1/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "prod-east-appdiscoveryService-nlb" {
  source                           = "../../../../../modules/nlb"
...
#Workspace naming format is adsds_nlb_ae_(base date)_(iteration number)