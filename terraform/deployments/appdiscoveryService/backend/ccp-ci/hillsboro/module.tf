terraform {
  backend "s3" {
    ...
    key    = "coast/ccp-ci/dataService/adsds/ccp-ho-az1/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "ccp-ci-ho-adsds" {
  ...
  comcast_application_role = "adsds"
  comcast_application_env  = "ci"
  region                   = "hillsboro"
  data_center              = "th"
  az                       = "az1"
  description              = "ADS DS 2.56.16 - CI"
      ...
      runcmd:
          - |
            ...
            if [ -f TRIGGER_FILE ]; then
                ...
                cd "$(\ls -1dt ./*/ | head -n 1)"
                ansible-playbook -vvvv --connection=local --inventory "127.0.0.1," appdiscoveryService.yml --extra-vars "env_type=ci datacenter=th java_package=java-11-amazon-corretto-devel.x86_64 run_number=b" > local_2nd_ansible_run.txt
