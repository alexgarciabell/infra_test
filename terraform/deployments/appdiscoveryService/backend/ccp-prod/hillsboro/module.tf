terraform {
  backend "s3" {
    ...
    key    = "coast/ccp-prod/dataService/adsds/ccp-ho-az1/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "ccp-prod-ho-adsds" {
  ...
  data_center              = "ph"
  az                       = "az1"
  description              = "ADS DS 2.56.16 - PROD"
      ...
      runcmd:
          - |
            ...
            if [ -f TRIGGER_FILE ]; then
                ...
                cd "$(\ls -1dt ./*/ | head -n 1)"
                ansible-playbook -vvvv --connection=local --inventory "127.0.0.1," appdiscoveryService.yml --extra-vars "env_type=prod datacenter=ph java_package=java-1.8.0-openjdk.x86_64 run_number=b" > local_2nd_ansible_run.txt
