terraform {
  backend "s3" {
    ...
    key    = "coast/ccp-prod/dataService/adsds/ccp-ch-az1/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "ccp-prod-ch-adsds" {
      ...
      runcmd:
          - |
            ...
            if [ -f TRIGGER_FILE ]; then
                ...
                cd "$(\ls -1dt ./*/ | head -n 1)"
                ansible-playbook -vvvv --connection=local --inventory "127.0.0.1," appdiscoveryService.yml --extra-vars "env_type=prod datacenter=pc java_package=java-1.8.0-openjdk.x86_64 run_number=b" > local_2nd_ansible_run.txt
