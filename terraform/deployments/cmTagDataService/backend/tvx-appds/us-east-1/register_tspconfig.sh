...
echo $payload
echo ====================
request="curl -H \"User-Agent: manual_tsp_host_upload\" -H \"Accept: application/json\" -H \"Content-Type: application/json\" -H \"Authorization: Bearer $token\" -X PUT -d $payload https://current.adsadmin.coast.xcal.tv/appdiscoveryAdminService/data/TSPConfig/cmTagDataService/"$VERSION"/"$ENVNAME"";
echo $request
eval $request
