terraform {
  backend "s3" {
    ...
    key    = "coast/ccp-prod/dataService/tagds/ccp-as-az1/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "ccp-prod-as-tagds" {
      ...
      runcmd:
          - |
            ...
            if [ -f TRIGGER_FILE ]; then
                ...
                cd "$(\ls -1dt ./*/ | head -n 1)"
                ansible-playbook -vvvv --connection=local --inventory "127.0.0.1," cmTagDataService.yml --extra-vars "env_type=prod datacenter=pa run_number=b" > local_2nd_ansible_run.txt
