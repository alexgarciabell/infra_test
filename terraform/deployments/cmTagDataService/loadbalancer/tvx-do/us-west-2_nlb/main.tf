terraform {
  backend "s3" {
    ...
    key    = "coast/ci/nlb/cmTagDataService/us-west-2/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "ci-west-cmtagds-nlb" {
  source                           = "../../../../../modules/nlb"
  ...
  description                      = "NLB For cmTagDataService - CI"
...
#Workspace naming format is tagds_nlb_dw_(base date)_(iteration number)