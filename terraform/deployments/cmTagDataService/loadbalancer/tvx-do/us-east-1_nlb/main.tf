terraform {
  backend "s3" {
    ...
    key    = "coast/ci/nlb/cmTagDataService/us-east-1/tfstate/terraform.tfstate"
    region = "us-west-2"
  }
}

module "ci-east-cmtagds-nlb" {
  source                           = "../../../../../modules/nlb"
...
#Workspace naming format is tagds_nlb_de_(base date)_(iteration number)