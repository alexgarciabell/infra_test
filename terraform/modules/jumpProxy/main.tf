...
data "aws_ami" "jumpProxy_ami" {
  most_recent = true

  filter {
    name   = "name"
    values = ["icfar-jumpproxy-*"]
  }
...
resource "aws_instance" "single_subnet_dataService_instance" {
  ...
  ami                     = "${data.aws_ami.jumpProxy_ami.id}"
  ...
  tags = {
    Name                          = "${var.name}-${var.data_center}-${random_string.stacknamestring.result}-jump-${count.index}.${var.domain}"
