...
if [[ $# -lt 2 ]]; then
  echo "usage: hatsmakerv2.sh <version> <account> <dry_run OR real_run>"
  echo "version = 1.2.3[-SNAPSHOT]"
  echo "account = appds | do | oi_prod | oi_ci"
...
if [[ "${account}" == "appds" ]]; then
  ...
  if [[ "${account}" == "oi_prod" ]]; then
    vpc_id="vpc-0cced001c67fa7bbc"
    subnet="subnet-05952e54697cef75e"
    region="us-west-2"
    vault_path=/secret/appds/aws/oi_prod/iam_user_creds
  fi
else
  if [[ "${account}" == "oi_ci" ]]; then
    vpc_id="vpc-02e9673bf44ffa93e"
    subnet="subnet-0621c0380e1fcc725"
    region="us-west-2"
    vault_path=/secret/appds/aws/oi_ci/iam_user_creds
  fi
fi
...
echo "Building hatsmakerv2 Packer template."
